...
<p>
   ...
   <center>
      ...
      onclick="get_search_invoiceToUpdate(<?php echo $num_fact;?>);">Modifier</button>
             <?php if($this->session->userdata('droit') < 4):?>
   ...
   <?php else:?>
            <center>
            <?php if($this->session->userdata('droit') < 4):?>
              <button type="submit" name="save" class="btn" style="border:1px solid #3445b4; color: #3445b4; font-size:12px;"
               onclick="doPrintPatientReceiptPdf(<?php echo $num_fact;?>,'<?php echo $date_fact;?>'); opener.location.reload(); window.close();">Imprimer le recu
             </button>
            <?php endif;?>
            </center>
         </p>
